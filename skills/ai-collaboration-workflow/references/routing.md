...
Create one `WORK-*` with `python3 scripts/workflow_task.py new <slug> --route tracked`. Keep approach, slices, validation, inline review, checkpoints, and experience candidates in that file. Use the template manually only when the helper is unavailable.
...
Start with `python3 scripts/workflow_task.py new <slug> --route governed`. Create:
